document.addEventListener('DOMContentLoaded', function () {
    ...
    const isLoginPage = Boolean(document.querySelector('[data-login-page]')) || window.location.pathname.includes('/auth/login.php');
    const installBannerDismissKey = 'expenseTrackerInstallBannerDismissed';
    ...
    const createInstallBanner = function (message, installLabel, onInstallClick) {
        if (!isLoginPage) {
            return;
        }
        ...
        banner.innerHTML = ''
            ...
            + '<div class="install-banner-actions"></div>';
        ...
        if (installLabel !== '') {
            ...
            installButton.className = 'button install-banner-install';
        ...
        dismissButton.className = 'button secondary install-banner-dismiss';
    ...
    const showIosInstallHint = function () {
        if (!isLoginPage || !isIos || isStandalone || !isSafari) {
            return;
        }

        createInstallBanner('Install this app: tap Share, then "Add to Home Screen".', '', function () {});
    };
    ...
    window.addEventListener('beforeinstallprompt', function (event) {
        ...
        createInstallBanner('Install Expense Tracker for faster access.', 'Install App', async function () {
            ...
            const existingBanner = document.querySelector('[data-install-banner]');
            if (existingBanner) {
    ...
    // Fallback: always show guidance on login when not installed.
    window.setTimeout(function () {
        if (!isLoginPage || isStandalone || document.querySelector('[data-install-banner]')) {
            return;
        }
        ...
        createInstallBanner('Install this app from your browser menu: Share/Menu -> Add to Home Screen.', '', function () {});
    }, 800);
